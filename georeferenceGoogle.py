...
# Leer archivo Excel
archivo = '/home/alex/Documentos/empresas_listado.xlsx'  # Especificar la ruta del archivo
...
for i, direccion in enumerate(df['DireccionCompleta']):
    # Continua solo si la dirección no está vacía
    if not direccion.strip():
        print(f"Dirección vacía en el registro {i + 1}")
        coordenadas.append([None, None])
        continue

    lat, lng = obtener_coordenadas_google(direccion, api_key)
    ...
    # Guardar progreso temporal cada 500 registros
    if (i + 1) % 500 == 0:
        ...
        df_temp.to_excel(f'/home/alex/Documentos/archivo_con_coordenadas_parcial_{i + 1}.xlsx', index=False) # Modificar RUTA para el archivo
        df_temp_json = df_temp[['id', 'Nombre', 'Documento', 'Delegación', 'DireccionCompleta', 'Latitud', 'Longitud']].to_dict(orient='records')
        with open(f'/home/alex/Documentos/archivo_con_coordenadas_parcial_{i + 1}.json', 'w', encoding='utf-8') as f: # Modificar RUTA para el archivo
            json.dump(df_temp_json, f,ensure_ascii=False, indent=4)  
...
df.to_excel('/home/alex/Documentos/archivo_con_coordenadas_google.xlsx', index=False) # Modificar RUTA para el archivo

# Guardar el resultado final en JSON
df_json = df[['id', 'Nombre', 'Documento', 'Delegación', 'DireccionCompleta', 'Latitud', 'Longitud']].to_dict(orient='records')
with open('/home/alex/Documentos/archivo_con_coordenadas_google.json', 'w', encoding='utf-8') as f: # Modificar RUTA para el archivo
